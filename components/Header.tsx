...
const NavItem: React.FC<{ to: string; children: React.ReactNode; onClick: () => void }> = ({ to, children, onClick }) => {
    return (
        <NavLink
            ...
            onClick={onClick}
            className={({ isActive }) =>
                `block md:inline-block px-4 py-2 md:py-0 text-base md:text-sm uppercase font-display tracking-wider transition-colors duration-300 ${
                isActive ? 'text-primary' : 'text-light-text hover:text-primary'
                }`
            }
        >
            {children}
...
const Header: React.FC = () => {
    ...
    return (
        <header className="bg-white/80 backdrop-blur-sm sticky top-0 z-50 shadow-md border-b border-gray-200">
            <div className="container mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
                <NavLink to="/" className="flex items-center gap-2 text-xl sm:text-2xl font-display font-bold text-dark-text tracking-widest">
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="md:hidden bg-white absolute w-full left-0 top-full shadow-lg"
                    >
